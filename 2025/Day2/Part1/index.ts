...
// id is invalid when starting with 0 or when it is made up of two identical sequences
for (const [start, end] of allRanges) {
  for (let id = start; id <= end; id++) {
    ...
    // split the id into 2 sequences
    const idStr = id.toString();
    const mid = Math.floor(idStr.length / 2);
    const seq1 = idStr.slice(0, mid);
    const seq2 = idStr.slice(mid);
    if (seq1 === seq2) {
      invalidIds.add(id);
